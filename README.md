...
| **DOCX Native + Fallback thông minh** | Word có text thật được đọc trực tiếp từ `word/document.xml`, `numbering.xml`, `styles.xml`; nhận cả lựa chọn A/B/C/D viết tay, `(A)`, auto-list của Word, marker highlight/vàng/chữ đỏ/shading/tick; nếu thiếu marker đáp án vẫn chia theo block câu để AI xử lý đủ. |
...
| **DOCX Native Parser** | Đọc `word/document.xml`, `word/numbering.xml`, `word/styles.xml`, giữ highlight vàng/chữ đỏ/shading/tick, hiểu auto-list của Word, tách MCQ 4/5 lựa chọn và chia batch 10 câu; structured fallback không còn kẹt 1 batch khi file dưới 15.000 ký tự. |
