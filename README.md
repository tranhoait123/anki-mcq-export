...
| **DOCX Native + Fallback thông minh** | Word có text thật được đọc trực tiếp từ `word/document.xml`, nhận cả lựa chọn A/B/C/D viết tay và danh sách tự động của Word, giữ highlight vàng hoặc chữ đỏ làm đáp án đúng; Word scan sẽ được cảnh báo nên chuyển PDF/ảnh. |
...
| **DOCX Native Parser** | Đọc `word/document.xml`, giữ highlight vàng/chữ đỏ, hiểu auto-list của Word, tách MCQ 4/5 lựa chọn và chia batch 10 câu. |
