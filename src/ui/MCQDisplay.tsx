...
const MCQDisplay: React.FC<MCQDisplayProps> = ({ mcqs, onUpdate, onDelete }) => {
  ...
  const filtered = useMemo(() =>
    ...
    [mcqs, searchTerm, showWarningsOnly]);

  const handleEdit = (mcq: MCQ) => {
    setEditingId(mcq.id);
    setEditForm(JSON.parse(JSON.stringify(mcq))); // Deep copy
    setViewMode('edit');
  };
  ...
  const handleExplanationChange = (field: keyof Explanation, value: string) => {
    ...
    setEditForm({
      ...editForm,
      explanation: { ...editForm.explanation, [field]: value }
    });
  };
  ...
  return (
    <div className="space-y-8">
      <div className="glass p-3 rounded-2xl sticky top-16 z-40 flex items-center gap-4 transition-all hover:shadow-lg">
        <div className="flex-1 flex items-center gap-3 px-3 py-2 bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-transparent focus-within:border-indigo-500/50 transition-all">
      ...
      <div className="space-y-8 pb-20">
        {filtered.map((mcq, idx) => {
          ...
          if (viewMode === 'preview' && !isEditing) {
            ...
            return (
              <div key={mcq.id} className="glass rounded-3xl p-8 pro-shadow transition-all group hover:-translate-y-1">
                  ...
                  >
                    ...
                    {isEditing ? (
                      <div className="flex-1 flex gap-3 items-center">
                        ...
                        />
                        <div className="relative flex items-center cursor-pointer" onClick={() => handleChange('correctAnswer', opt)}>
                  ...
                  <div className="space-y-5 text-sm">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        ...
                        <textarea className="w-full bg-rose-50/50 dark:bg-rose-900/10 border-none rounded-2xl p-4 focus:ring-2 focus:ring-rose-500" rows={3} value={data.explanation.core} onChange={e => handleExplanationChange('core', e.target.value)} />
                      </div>
                      <div>
                        <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-2 block">Phân tích sâu</label>
                        <textarea className="w-full bg-indigo-50/50 dark:bg-indigo-900/10 border-none rounded-2xl p-4 focus:ring-2 focus:ring-indigo-500" rows={3} value={data.explanation.analysis} onChange={e => handleExplanationChange('analysis', e.target.value)} />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Bằng chứng tài liệu</label>
                      <textarea className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl p-4 focus:ring-2 focus:ring-slate-400" rows={2} value={data.explanation.evidence} onChange={e => handleExplanationChange('evidence', e.target.value)} />
                    </div>
                    <div className="flex gap-3 justify-end pt-4">
                      <button onClick={() => setEditingId(null)} className="px-6 py-2.5 text-slate-500 font-bold text-xs bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 transition-all">Hủy bỏ</button>
                      <button onClick={handleSave} className="px-8 py-2.5 text-white font-bold text-xs pro-gradient rounded-xl shadow-lg shadow-indigo-200 dark:shadow-none hover:scale-105 transition-all">Lưu thay đổi</button>
                    </div>
                  </div>
                ) : (
                  <>
